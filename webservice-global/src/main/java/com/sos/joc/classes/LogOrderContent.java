...
public class LogOrderContent extends LogContent {
	...
	public Path writeGzipLogFile() throws Exception {
		...
		if (path == null) {
			String msg = String.format("Order log of %s,%s with id %s is missing", orderHistoryFilter.getJobChain(),
	...
	private Path writeGzipLogFileFromDB() throws DBMissingDataException, IOException, NumberFormatException,
			SOSHibernateException, JocConfigurationException {
		SOSHibernateSession sosHibernateSession = null;
		try {
			SOSHibernateFactory sosHibernateFactory = Globals
					.getHibernateFactory(orderHistoryFilter.getJobschedulerId());
			sosHibernateSession = sosHibernateFactory.openStatelessSession("getOrderLog");
			try {
				Globals.beginTransaction(sosHibernateSession);
				JobSchedulerOrderHistoryDBLayer jobSchedulerOrderHistoryDBLayer = new JobSchedulerOrderHistoryDBLayer(
						sosHibernateSession);
	...
	// String logFilename = String.format("order.%s.%s.log",
	// orderHistoryFilter.getJobChain().replaceFirst("^/+", "").replaceAll("/",
	// ","),
	...
	private Path writeGzipOrderLogFileFromXmlCommand() throws Exception {
		...
		try {
			...
			if (clusterMemberId != null && !clusterMemberId.isEmpty()) {
				if (!clusterMemberId.equals(dbItemInventoryInstance.clusterMemberId())) {
					sosHibernateSession = Globals.createSosHibernateStatelessConnection("getOrderLog");
					InventoryInstancesDBLayer instancesDbLayer = new InventoryInstancesDBLayer(sosHibernateSession);
					DBItemInventoryInstance itemInventoryInstance2 = instancesDbLayer
							.getInventoryInstanceByClusterMemberId(clusterMemberId);
		...
		String xml = jocXmlCommand.getShowOrderCommand(orderHistoryFilter.getJobChain(),
				orderHistoryFilter.getOrderId(), "log");
		return jocXmlCommand.getLogPath(xml, getAccessToken(), getPrefix(), true);
	}

	private String getPrefix() {
		return String
				.format("sos-%s.%s.%s.order.log-download-",
						Paths.get(orderHistoryFilter.getJobChain()).getFileName().toString(),
						orderHistoryFilter.getOrderId(), orderHistoryFilter.getHistoryId())
				.replace(',', '.').replaceAll("[/\\\\:;*?!&\"'<>|^]", "");
	}
}
