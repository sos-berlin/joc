...
public class EventResourceImpl extends JOCResourceImpl implements IEventResource {
	...
	@Override
	public JOCDefaultResponse postEvent(String xAccessToken, String accessToken, RegisterEvent eventBody)
			throws Exception {
	...
	public JOCDefaultResponse postEvent(String accessToken, RegisterEvent eventBody) {
		...
		try {
			JOCDefaultResponse jocDefaultResponse = init(API_CALL, eventBody, accessToken, "",
			...
			for (JobSchedulerObjects jsObject : eventBody.getJobscheduler()) {
				...
				if (isCurrentJobScheduler) {
					...
					List<DBItemInventoryInstance> jobSchedulerMembers = instanceLayer
							.getInventoryInstancesBySchedulerId(jsObject.getJobschedulerId());
					if (instance.getSupervisorId() > 0) {
						DBItemInventoryInstance supervisor = instanceLayer
								.getInventoryInstanceByKey(instance.getSupervisorId());
						if (supervisor != null) {
							JobSchedulerEvent jsEventOfMember = initEvent(jsObject, session, supervisor.getId(),
									defaultEventId);
							JOCJsonCommand commandOfMember = initJocJsonCommand(jsEventOfMember, supervisor,
									"SchedulerEvent");
							jocJsonCommandsOfClusterMember.add(commandOfMember);
							tasksOfClusterMember.add(new EventCallableJobSchedulerStateChanged(commandOfMember,
									jsEventOfMember, accessToken, session, supervisor.getId()));
						} else {
							Boolean supervisorWarningIsLogged = (Boolean) session
									.getAttribute("supervisorWarningIsLogged");
							if (supervisorWarningIsLogged == null) {
								LOGGER.warn(String.format(
										"Supervisor with ID %d expected but not found in INVENTORY_INSTANCES",
										instance.getSupervisorId()));
					...
					for (DBItemInventoryInstance jobSchedulerMember : jobSchedulerMembers) {
						...
						JobSchedulerEvent jsEventOfMember = initEvent(jsObject, session, jobSchedulerMember.getId(),
								defaultEventId);
						JOCJsonCommand commandOfMember = initJocJsonCommand(jsEventOfMember, jobSchedulerMember,
								"SchedulerEvent");
						jocJsonCommandsOfClusterMember.add(commandOfMember);
						tasksOfClusterMember.add(new EventCallableJobSchedulerStateChanged(commandOfMember,
								jsEventOfMember, accessToken, session, jobSchedulerMember.getId()));
					}
					tasks.add(new EventCallableOfCurrentCluster(command, jsEvent, accessToken, session,
							instance.getId(), shiroUser, tasksOfClusterMember, jocJsonCommandsOfClusterMember,
							getNestedJobChains(jobChainLayer, instance)));
	...
	private JobSchedulerEvent initEvent(JobSchedulerObjects jsObject, Session session, Long instanceId,
			Long defaultEventId) {
	...
	private Map<String, Set<String>> getNestedJobChains(InventoryJobChainsDBLayer jobChainLayer,
	...
	private DBItemInventoryInstance getJobSchedulerInstance(JobSchedulerObjects jsObject,
			InventoryInstancesDBLayer instanceLayer, String accessToken)
			throws DBInvalidDataException, DBMissingDataException, DBConnectionRefusedException {
		DBItemInventoryInstance instance = setMappedUrl(
				Globals.urlFromJobSchedulerId.get(jsObject.getJobschedulerId()));
		if (instance == null) {
