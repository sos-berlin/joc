...
public class JobChainResourceHistoryImpl extends JOCResourceImpl implements IJobChainResourceHistory {
	...
	private SortedSet<OrderHistoryItem> handleJobChain(JOCXmlCommand jocXmlCommand,
			JobChainHistoryFilter jobChainHistoryFilter) throws Exception {

		SortedSet<OrderHistoryItem> orderHistoryItems = new TreeSet<OrderHistoryItem>(
				new Comparator<OrderHistoryItem>() {
	...
	public JOCDefaultResponse postJobChainHistory(String accessToken, byte[] jobChainHistoryFilterBytes) {
		...
		try {
			...
			NodeList jobChainNodes = jocXmlCommand.getSosxml().selectNodeList(jobChain,
					"job_chain_node/@job_chain | job_chain_node.job_chain/@job_chain");
	...
	private HistoryStateText getStatus(ReportTriggerDBLayer reportTriggerDBLayer,
			InventoryJobChainsDBLayer jobChainsLayer, OrderHistoryItem history) throws Exception {
		...
		if (listOfReportTriggerDBItems.size() > 0) {
			...
			return HistoryStateText.INCOMPLETE;
			// if (!jobChainsLayer.isErrorNode(history.getJobChain(), history.getNode(),
		...
		if (dbItemReportTrigger == null
				|| (dbItemReportTrigger.getStartTime() != null && dbItemReportTrigger.getEndTime() == null)) {
			return HistoryStateText.INCOMPLETE;
			// if (!jobChainsLayer.isErrorNode(history.getJobChain(), history.getNode(),
			// dbItemInventoryInstance.getId())) {
			// return HistoryStateText.SUCCESSFUL;
