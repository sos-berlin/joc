...
public class CalendarsImportResourceImpl extends JOCResourceImpl implements ICalendarsImportResource {
	...
	@Override
	public JOCDefaultResponse importCalendars(String accessToken, CalendarImportFilter calendarImportFilter)
			throws Exception {
		SOSHibernateSession connection = null;
		try {
			JOCDefaultResponse jocDefaultResponse = init(API_CALL, null, accessToken,
					calendarImportFilter.getJobschedulerId(),
					getPermissonsJocCockpit(calendarImportFilter.getJobschedulerId(), accessToken).getCalendar()
							.getEdit().isCreate());
			...
			if (calendars != null && !calendars.isEmpty()) {
				...
				for (Calendar calendar : calendars) {
					...
					if (calendar.getBasedOn() == null || calendar.getBasedOn().isEmpty()) {
						...
						ModifyCalendarAudit calendarAudit = new ModifyCalendarAudit(calendar.getId(),
								calendar.getPath(), calendarImportFilter.getAuditLog(),
								calendarImportFilter.getJobschedulerId());
						...
						if (dbItemCalendar == null) {
							...
							dbItemCalendar = importCalendar(calendar, dbItemCalendar, calendarDbLayer,
									calendarUsageDbLayer);
						}
						if (dbItemCalendar != null) {
				...
				for (Calendar calendar : calendars) {
					if (calendar.getBasedOn() != null && !calendar.getBasedOn().isEmpty()
						...
						case ORDER:
							objectExists = (ordersDBLayer.getInventoryOrderByName(dbItemInventoryInstance.getId(),
									calendar.getPath()) != null);
							break;
						case JOB:
							objectExists = (jobsDBLayer.getInventoryJobByName(calendar.getPath(),
									dbItemInventoryInstance.getId()) != null);
							break;
						case SCHEDULE:
							objectExists = (schedulesDBLayer.getSchedule(calendar.getPath(),
									dbItemInventoryInstance.getId()) != null);
							break;
						default:
							break;
						...
						DBItemInventoryCalendarUsage dbItemInventoryCalendarUsage = calendarUsageDbLayer
								.getCalendarUsageOfAnObject(dbItemInventoryInstance.getId(), calendar.getBasedOn(),
										calendar.getType().name(), calendar.getPath());
						if (dbItemInventoryCalendarUsage != null
								&& dbItemInventoryCalendarUsage.getConfiguration() != null) {
							continue;
						} else {
							importCalendarUsage(dbItemInventoryCalendarUsage, calendar, calendarUsageDbLayer,
									calendarsMap.get(calendar.getBasedOn()));
						}
					}
				}
				...
				if (updateJobOrderScheduleIsNecessary != null) {
					for (Calendar calendar : updateJobOrderScheduleIsNecessary) {
						if (calendarsMap.containsKey(calendar.getPath())) {
							CalendarUsagesAndInstance calendarUsageInstance = new CalendarUsagesAndInstance(
									dbItemInventoryInstance, false);
							List<DBItemInventoryCalendarUsage> dbCalendarUsages = calendarUsageDbLayer
									.getCalendarUsages(calendarsMap.get(calendar.getPath()).getId());
							if (!dbCalendarUsages.isEmpty()) {
								...
								JobSchedulerCalendarCallable callable = new JobSchedulerCalendarCallable(
										calendarUsageInstance, getAccessToken());
								...
								for (Entry<String, Exception> exception : c.getExceptions().entrySet()) {
									if (exception.getValue() instanceof JocException) {
										...
										listOfErrors.add(new BulkError().get(exception.getValue(), getJocError(),
												exception.getKey()));
									}
								}
							}
	...
	private void checkRequirements(Calendar calendar) throws JocMissingCommentException,
		...
		if (calendar.getBasedOn() == null || calendar.getBasedOn().isEmpty()) {
			if (calendar.getTo() == null || calendar.getTo().isEmpty()) {
				...
				throw new JobSchedulerInvalidResponseDataException(
						"calendar field 'to' must have the format YYYY-MM-DD.");
			}
	...
	private DBItemInventoryCalendarUsage importCalendarUsage(DBItemInventoryCalendarUsage dbItemInventoryCalendarUsage,
			Calendar calendar, CalendarUsageDBLayer calendarUsageDbLayer, DBItemCalendar dbItemCalendar)
		...
		eventCommands.add(SendCalendarEventsUtil.addCalUsageEvent(dbItemInventoryCalendarUsage.getPath(),
				dbItemInventoryCalendarUsage.getObjectType(), "CalendarUsageUpdated"));
	...
	private DBItemCalendar importCalendar(Calendar calendar, DBItemCalendar calendarDbItem,
			CalendarsDBLayer calendarDbLayer, CalendarUsageDBLayer calendarUsageDbLayer) {
		try {
			CalendarEvent calEvt = new CalendarEvent();
			CalendarVariables calEvtVars = new CalendarVariables();
			calEvtVars.setPath(calendar.getPath());
			...
			try {
				...
				if ((calendar.getType() != null && !calendar.getType().name().equals(calendarDbItem.getType()))) {
					List<DBItemInventoryCalendarUsage> dbCalendarUsages = calendarUsageDbLayer
							.getCalendarUsages(calendarDbItem.getId());
