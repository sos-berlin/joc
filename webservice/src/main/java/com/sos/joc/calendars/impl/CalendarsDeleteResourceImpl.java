...
public class CalendarsDeleteResourceImpl extends JOCResourceImpl implements ICalendarsDeleteResource {
	...
	@Override
	public JOCDefaultResponse postDeleteCalendars(String accessToken, CalendarsFilter calendarsFilter)
			throws Exception {
		SOSHibernateSession connection = null;
		try {
			JOCDefaultResponse jocDefaultResponse = init(API_CALL, calendarsFilter, accessToken,
					calendarsFilter.getJobschedulerId(),
					getPermissonsJocCockpit(calendarsFilter.getJobschedulerId(), accessToken).getCalendar().getEdit()
							.isDelete());
			...
			if (calendarsFilter.getCalendars() == null || calendarsFilter.getCalendars().size() == 0) {
				for (Long calendarId : new HashSet<Long>(calendarsFilter.getCalendarIds())) {
					executeDeleteCalendar(calendarDbLayer.getCalendar(calendarId), calendarsFilter, calendarDbLayer,
							accessToken);
				}
			} else {
				for (String calendarPath : new HashSet<String>(calendarsFilter.getCalendars())) {
					executeDeleteCalendar(calendarDbLayer.getCalendar(dbItemInventoryInstance.getId(), calendarPath),
							calendarsFilter, calendarDbLayer, accessToken);
				}
			}
	...
	private void executeDeleteCalendar(DBItemCalendar calendarDbItem, CalendarsFilter calendarsFilter,
			CalendarsDBLayer calendarDbLayer, String accessToken) {
		if (calendarDbItem != null) {
			try {
				ModifyCalendarAudit calendarAudit = new ModifyCalendarAudit(calendarDbItem.getId(),
						calendarDbItem.getName(), calendarsFilter.getAuditLog(), calendarsFilter.getJobschedulerId());
