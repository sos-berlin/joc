	...
	public JOCDefaultResponse postCalendarDates(String accessToken, byte[] calendarFilterBytes)
		...
		try {
			...
			if (calendarPathIsDefined || calendarIdIsDefined) {
				...
				calendarFilter
						.setCalendar(new ObjectMapper().readValue(calendarItem.getConfiguration(), Calendar.class));

			}

			try {
				if (calendarFilter.getCalendar().getBasedOn() != null
						&& !calendarFilter.getCalendar().getBasedOn().isEmpty()) {
					connection = Globals.createSosHibernateStatelessConnection(API_CALL);
					CalendarsDBLayer dbLayer = new CalendarsDBLayer(connection);
					...
					Calendar basedCalendar = new ObjectMapper().readValue(calendarItem.getConfiguration(),
							Calendar.class);
					if (calendarFilter.getDateFrom() == null || calendarFilter.getDateFrom().isEmpty()) {
						calendarFilter.setDateFrom(fr.getToday());
					}
					dates = fr.resolveRestrictions(basedCalendar, calendarFilter.getCalendar(),
							calendarFilter.getDateFrom(), calendarFilter.getDateTo());
				} else {
