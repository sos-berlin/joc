...
public class JobsResourceStartJobsImpl extends JOCResourceImpl implements IJobsResourceStartJob {
	...
	@Override
	public JOCDefaultResponse postJobsStart(String xAccessToken, String accessToken, StartJobs startJobs)
			throws Exception {
	...
	public JOCDefaultResponse postJobsStart(String accessToken, StartJobs startJobs) throws Exception {
		try {
			JOCDefaultResponse jocDefaultResponse = init(API_CALL, startJobs, accessToken,
					startJobs.getJobschedulerId(), getPermissonsJocCockpit(startJobs.getJobschedulerId(), accessToken)
							.getJob().getExecute().isStart());
		...
		try {
			...
			if (startJob.getAt() == null || startJob.getAt().isEmpty()) {
				...
				if (startJob.getAt().contains("now")) {
					...
					xml.addAttribute("at", JobSchedulerDate.getAtInJobSchedulerTimezone(startJob.getAt(),
