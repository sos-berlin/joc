...
public class JobSchedulerResourceModifyJobSchedulerClusterImpl extends JOCResourceImpl
	...
	public JOCDefaultResponse postJobschedulerTerminate(String xAccessToken, String accessToken,
			TimeoutParameter timeoutParameter) throws Exception {
		return postJobschedulerTerminate(getAccessToken(xAccessToken, accessToken), timeoutParameter);
	}

	public JOCDefaultResponse postJobschedulerTerminate(String accessToken, TimeoutParameter timeoutParameter)
			throws Exception {
		try {
			boolean permission = getPermissonsJocCockpit(timeoutParameter.getJobschedulerId(), accessToken)
					.getJobschedulerMasterCluster().getExecute().isTerminate();
	...
	public JOCDefaultResponse postJobschedulerRestartTerminate(String accessToken, TimeoutParameter timeoutParameter)
			throws Exception {
		try {
			boolean permission = getPermissonsJocCockpit(timeoutParameter.getJobschedulerId(), accessToken)
					.getJobschedulerMasterCluster().getExecute().isRestart();
	...
	@Override
	public JOCDefaultResponse postJobschedulerTerminateFailSafe(String xAccessToken, String accessToken,
			TimeoutParameter timeoutParameter) throws Exception {
		return postJobschedulerTerminateFailSafe(getAccessToken(xAccessToken, accessToken), timeoutParameter);
	}

	public JOCDefaultResponse postJobschedulerTerminateFailSafe(String accessToken, TimeoutParameter timeoutParameter)
			throws Exception {
		try {
			boolean permission = getPermissonsJocCockpit(timeoutParameter.getJobschedulerId(), accessToken).getJobschedulerMasterCluster().getExecute()
					.isTerminateFailSafe();
			return executeModifyJobSchedulerClusterCommand(TERMINATE_FAILSAFE, timeoutParameter, accessToken,
					permission);
		} catch (JocException e) {
			e.addErrorMetaInfo(getJocError());
			return JOCDefaultResponse.responseStatusJSError(e);
	...
	private JOCDefaultResponse executeModifyJobSchedulerClusterCommand(String command,
			TimeoutParameter timeoutParameter, String accessToken, boolean permission) throws Exception {
		JOCDefaultResponse jocDefaultResponse = init(API_CALL + command, timeoutParameter, accessToken,
				timeoutParameter.getJobschedulerId(), permission);
