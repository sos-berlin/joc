	...
	public JOCDefaultResponse postJobschedulerCleanupInventory(String accessToken, HostPortParameter hostPortParameter)
		...
		try {
			...
			JOCDefaultResponse jocDefaultResponse = init(API_CALL, hostPortParameter, accessToken,
					...
					getPermissonsJocCockpit(hostPortParameter.getJobschedulerId(), accessToken).getJobschedulerMaster()
			...
			DBItemInventoryInstance schedulerInstanceFromDb = instanceLayer.getInventoryInstanceByHostPort(
					hostPortParameter.getHost(), hostPortParameter.getPort(), hostPortParameter.getJobschedulerId());
			...
			if (hostPortParameter.getJobschedulerId().equals(dbItemInventoryInstance.getSchedulerId())
					&& hostPortParameter.getHost().equals(dbItemInventoryInstance.getHostname())
					&& hostPortParameter.getPort() == dbItemInventoryInstance.getPort()) {
				try {
