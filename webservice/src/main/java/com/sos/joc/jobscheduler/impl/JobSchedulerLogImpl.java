...
public class JobSchedulerLogImpl extends JOCResourceImpl implements IJobSchedulerLogResource {
    ...
    @Override
    public JOCDefaultResponse getMainLog(String xAccessToken, String accessToken, HostPortTimeOutParameter hostPortParamSchema) throws Exception {
    ...
    public JOCDefaultResponse getMainLog(String accessToken, HostPortTimeOutParameter hostPortParamSchema) throws Exception {
        ...
        try {
            ...
            // LOGGER.info(str);
            // str = str.replaceFirst("^.*\n<pre class='log'>", "").replaceAll("<span class='[^']+'>(\\d{4}-\\d{2}-\\d{2})",
            // "$1").replaceAll("</span>(\r?\n)","$1");
            // LOGGER.info(str);
