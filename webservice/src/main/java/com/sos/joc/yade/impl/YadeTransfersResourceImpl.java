...
public class YadeTransfersResourceImpl extends JOCResourceImpl implements IYadeTransfersResource {
	...
	public JOCDefaultResponse postYadeTransfers(String accessToken, TransferFilter filterBody) throws Exception {
		...
		try {
			...
			List<DBItemYadeTransfers> transfersFromDb = dbLayer.getFilteredTransfers(filterBody.getTransferIds(),
					operationValues, stateValues, filterBody.getMandator(), sourceHosts, sourceProtocols, targetHosts,
					targetProtocols, filterBody.getIsIntervention(), filterBody.getHasIntervention(),
