...
public class Events {
    ...
    public void put(EventSnapshot event) {
        if ("AuditLogChanged".equals(event.getEventType())) {
            ...
            this.events.putIfAbsent(event.getEventType() + "." + event.getPath() + "." + event.getObjectType().name(), event);
        } else if (event.getEventType().startsWith("SchedulerStateChanged")) {
            this.events.putIfAbsent(event.getEventType() + "." + event.getPath() + "." + event.getObjectType().name(), event);
            ...
            this.events.put(event.getEventType() + "." + event.getPath() + "." + event.getObjectType().name(), event);
        } else if (event.getNodeId() != null && event.getTaskId() != null && event.getEventType().equals(JobSchedulerJobStreamsEventHandler.CustomEventType.TaskEnded.name())) {
            this.events.put(event.getTaskId() + "." + event.getNodeId() + "." + event.getPath() + "." + event.getObjectType().name(), event);
            // Taskend Event from JobStreams.
        } else {
