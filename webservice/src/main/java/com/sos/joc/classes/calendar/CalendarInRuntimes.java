...
public class CalendarInRuntimes {
    ...
    public static Map<String, Exception> delete(DBItemInventoryInstance dbItemInventoryInstance, Long calendarId, SOSHibernateSession connection, String accessToken) throws Exception {
        return exec(dbItemInventoryInstance, calendarId, connection, accessToken, null, false);
    }

    public static Map<String, Exception> update(DBItemInventoryInstance dbItemInventoryInstance, DBItemCalendar calendarDbItem, Dates newDates, SOSHibernateSession connection, String accessToken) throws Exception {
        if (calendarDbItem != null) {
            List<String> dates = new ArrayList<String>();
            if (newDates != null) {
                dates = newDates.getDates();
            }
            return exec(dbItemInventoryInstance, calendarDbItem.getId(), connection, accessToken, dates, true);
        }
        return new HashMap<String, Exception>();
    }

    private static Map<String, Exception> exec(DBItemInventoryInstance dbItemInventoryInstance, Long calendarId, SOSHibernateSession connection, String accessToken, List<String> dates, boolean update)
            throws Exception {
