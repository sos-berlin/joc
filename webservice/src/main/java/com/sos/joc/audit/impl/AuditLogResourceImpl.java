...
public class AuditLogResourceImpl extends JOCResourceImpl implements IAuditLogResource {
	...
	public JOCDefaultResponse postAuditLog(String accessToken, AuditLogFilter auditLogFilter) throws Exception {
		...
		try {
			...
			JOCDefaultResponse jocDefaultResponse = init(API_CALL, auditLogFilter, accessToken,
					auditLogFilter.getJobschedulerId(),
					getPermissonsJocCockpit(auditLogFilter.getJobschedulerId(), accessToken).getAuditLog().getView()
							.isStatus());
			...
			if (filterOrders != null && !filterOrders.isEmpty()) {
				auditLogs = dbLayer.getAuditLogByOrders(schedulerId, filterOrders, filterLimit, filterFrom, filterTo,
						filterTicketLink, filterAccount);
				...
				auditLogs = dbLayer.getAuditLogByCalendars(schedulerId, filterCalendars, filterLimit, filterFrom,
						filterTo, filterTicketLink, filterAccount);
			...
			if (filterRegex != null && !filterRegex.isEmpty() && (filterOrders == null || filterOrders.isEmpty())
					&& (filterJobs == null || filterJobs.isEmpty())) {
	...
	@Override
	public JOCDefaultResponse postAuditLog(String xAccessToken, String accessToken, AuditLogFilter auditLogFilter)
			throws Exception {
